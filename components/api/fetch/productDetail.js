    ...
    try {
        ...
        res.json().then(api => {
            ...
            $('.rating-count').each(function(i, el){
                $(this).html(rating_count(api.commentsCount.countPerRating,5-i,api.comments.count))
            });
            $('.reviews').empty();
            if(!api.comments.count) $('.reviews').html('<div class="text-center"><p>No comment for this product.</p><p>Be the first one now!</p></div>')
...
submit_comment.addEventListener("click", async function(e) {
    ...
    try {
        ...
        if(!res.ok) {
            const message = "Error with status code: " + res.status;
            res.json().then(message => document.querySelector('#warning-message').textContent = message.message);
            throw new Error(message);
        }
        getComments(0);
        if(!$('input[name=review_name]').attr('readonly'))
